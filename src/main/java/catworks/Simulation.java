...
public class Simulation {
    ...
    public int[][] run() throws IOException {
        ...
        // BufferedWriter writer = new BufferedWriter(new FileWriter(file)); // TODO:
        ...
        for (int timeStep = 0; timeStep < time; timeStep++) {
            ...
            data[timeStep][0] = timeStep;
            data[timeStep][1] = state.length;
            data[timeStep][2] = getNumInfected(state);
            data[timeStep][3] = immuneNum;
            // log("row = {" + row[0] + ", "+ row[1] + ", "+ row[2] + ", "+ row[3] + "}");
            // outputCSVRow(writer, row); //TODO
